<template>
  <div class="button smallButton float-left ml-2 ml-lg-5" :class="(connected) ? 'btConnected' : ''" @click="toggleBTConnection">
    <img class="smallIcon" src="@/assets/img/bluetoothOff.png" alt="bluetoothIcon" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    connect() {
      this.$store.dispatch("webBluetooth/addDevice", this.options)  // Find the device we are looking for with the provided options.
      .then(() => {
        let deviceID = "";
        for (let device of this.devices) {  // Go through all devices in webBluetooth devices state
          if (device.name.startsWith(this.options.namePrefix)) {  // Find the first one that starts with our prefix.
        ...
        this.$store.dispatch("webBluetooth/connectDevice", this.deviceOptions)  // Make a Connection to the Bluetooth Device TODO: this only works on the second attempt when first connecting for some reason, fix this.
        .then(() => {
